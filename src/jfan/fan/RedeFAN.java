...
public class RedeFAN implements Serializable, Cloneable {
	...
	/**
	 * Quantos erros aconteceram no geral
	 */
	//private transient int erros;
	
	/**o n�mero de erros de cada classe.
	 */
	//private transient int[] errosClasses;
	
	//private transient int[] errosClassesValidacao;
	...
	public RedeFAN(int raioDifuso, int suporteConjuntosDifusos, int numeroCaracteristicas, int numeroClasses) {
		...
		//Preparar neurônios
		neuronios = new NeuronioFAN[numeroClasses]; //1 neurônios para cada classe
		for(int i = 0; i < neuronios.length; i++) {
	...
	public void treinar(Collection<PadraoFAN> conjuntoTreinamento) {
		for(PadraoFAN p : conjuntoTreinamento)
			treinar(p);
	}
	...
	public void treinar(PadraoFAN p) {
		int respostaRede = classificar(p);
			...
			//tem que reforçar o original de p
	...
	protected void penalizar(NeuronioFAN neuronio, PadraoFAN p) {
		...
		for (int i = 0; i < carateristicas; i++) {
			...
			for(int j = caracFan.getInicio(); j < fim; j++) {
				...
				//Para sincronizar com o uso do método setTempera
				synchronized (lockTempera) {
					//Se a tempera não for nula, usa-lá.
					alpha = 1.0;
				...
				//Abaixo está como o copiado da implmentação em C... no mínimo estranho
				/*if (random.nextInt(100)>10) {
					novoValor *= Math.pow(conjDifuso[k],0.00001);
				}
				else {
					novoValor -=  conjDifuso[k];
				}*/
				
				//Filipe: implementado com base na tese de doutorado.
